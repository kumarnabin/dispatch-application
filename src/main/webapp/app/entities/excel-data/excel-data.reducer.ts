...
const apiUrl = 'api/excel-data';

// Actions

export const getEntities = createAsyncThunk('excelData/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiUrl}?${sort ? `page=${page}&size=${size}&sort=${sort}&` : ''}cacheBuster=${new Date().getTime()}`;
  return axios.get<IExcelData[]>(requestUrl);
});

export const getEntity = createAsyncThunk(
  'excelData/fetch_entity',
...
export const partialUpdateEntity = createAsyncThunk(
  'excelData/partial_update_entity',
  async (entity: IExcelData, thunkAPI) => {
